...
Basket.prototype.add = function(P_id,PName, Price, Quantity){
    this.productArray.push({id:P_id,name: PName, price: Price, quantity: Quantity});
    this.send();
    this.loadBasket();
...
Basket.prototype.remove = function(index){
    ...
    this.send(); 
    this.loadBasket();
    alert("Your product has been successfully removed from cart");
};

//Deletes all products from basket
Basket.prototype.empty = function (index){
    this.productArray.splice(index);
    this.send(); 
    this.loadBasket();
    alert("Your cart is now empty");
}

//Display checkout confirmation to customer.
Basket.prototype.checkout = function (index){
    this.productArray.splice(index);
    this.send(); 
    this.loadBasket();
...
Basket.prototype.loadBasket = function(){
    ...
    for(var i=0; i<this.productArray.length; ++i){
        htmlStr += "<p>" +  this.productArray[i].name +  ": " + this.productArray[i].quantity + "&nbsp <button class='button_1' type='submit' value='Remove' onclick='basket.remove(" + i + ")'>Remove From Cart</button>" + "</p>";
    }
